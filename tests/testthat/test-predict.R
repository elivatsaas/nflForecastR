...
test_that("predict_with_model generates correct predictions", {
  ...
  expect_true(all(c("home_team", "away_team", "predicted_point_differential",
                    "away.spread_line", "chosen_spread") %in% names(predictions)))
...
test_that("predict_with_model handles missing values appropriately", {
  skip_if_not_installed("ranger")
  ...
  test_data <- data.frame(
    feature1 = c(rnorm(9), NA),
    feature2 = c(rnorm(8), NA, NA),
    home_team = rep("KC", 10),
    away_team = rep("LV", 10),
    away.spread_line = rep(-3, 10)
  )
...
test_that("predict_with_model handles type conversions correctly", {
  ...
  result <- predict_with_model(test_data, rf_model,train_data)
  expect_equal(nrow(result), 10)
  expect_true(all(!is.na(result$predicted_point_differential)))
})
